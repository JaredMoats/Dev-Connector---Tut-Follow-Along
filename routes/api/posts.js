...
router.get("/:id", auth, async (req, res) => {
	try {
		/* Get post by ID. Pulling ID from the URL */
		const post = await Post.findById(req.params.id);

		//check to see if post with that id
		if (!post) {
...
router.put("/unlike/:id", auth, async (req, res) => {
	try {
		...
		//Check if the post has already been liked by current user
		/* If post equal to zero, user has not liked it  */
		if (
...
router.delete("/comment/:id/:comment_id", auth, async (req, res) => {
	try {
		...
		//Check if user owns comment before it is deleted
